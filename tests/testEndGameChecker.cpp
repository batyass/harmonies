...
using harmonies::rules::isGameOver;
...
int main()
{
    ...
    std::vector<const PersonalBoard *> boards = { player1.getBoard() };

    // Test 1: Initial state
    check(!isGameOver(boards, bag),
    ...
    harmonies::model::PersonalBoard *board = player1.getBoard();
    const std::map<harmonies::utils::HexCoord, harmonies::model::BoardCell> &cells = board->getCells();
    
    // Fill the board until only 3 empty spaces remain
    int fillCount = 0;
    int totalCells = static_cast<int>(cells.size());
    for (std::map<harmonies::utils::HexCoord, harmonies::model::BoardCell>::const_iterator it = cells.begin(); it != cells.end(); ++it) {
        const harmonies::utils::HexCoord &coord = it->first;
    ...
    check(!isGameOver(boards, fullBag),
